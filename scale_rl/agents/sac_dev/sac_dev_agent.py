...
from scale_rl.agents.sac_dev.sac_dev_network import (
    SACDevActor,
    SACDevClippedDoubleCritic,
    SACDevCritic,
    SACDevTemperature,
)
from scale_rl.agents.sac_dev.sac_dev_update import (
    update_actor,
    update_critic,
...
class SACDevConfig:
    ...
    # dev
    actor_use_post_layernorm: bool
    actor_use_post_linear: bool
    actor_post_hidden_dim: int
    actor_use_simnorm: bool
    actor_simnorm_levels: bool
    actor_use_project_linear: bool
    actor_project_hidden_dim: int
    actor_use_project_activ: bool

    critic_use_post_layernorm: bool
    critic_use_post_linear: bool
    critic_post_hidden_dim: int
    critic_use_simnorm: bool
    critic_simnorm_levels: bool
    critic_use_project_linear: bool
    critic_project_hidden_dim: int
    critic_use_project_activ: bool
...
def _init_sac_dev_networks(
    ...
    actor = Trainer.create(
        network_def=SACDevActor(
            block_type=cfg.actor_block_type,
            num_blocks=cfg.actor_num_blocks,
            hidden_dim=cfg.actor_hidden_dim,
            action_dim=action_dim,
            dtype=compute_dtype,
            use_post_layernorm=cfg.actor_use_post_layernorm,
            use_post_linear=cfg.actor_use_post_linear,
            post_hidden_dim=cfg.actor_post_hidden_dim,
            use_simnorm=cfg.actor_use_simnorm,
            simnorm_levels=cfg.actor_simnorm_levels,
            use_project_linear=cfg.actor_use_project_linear,
            project_hidden_dim=cfg.actor_project_hidden_dim,
            use_project_activ=cfg.actor_use_project_activ,
        ),
    ...
    if cfg.critic_use_cdq:
        critic_network_def = SACDevClippedDoubleCritic(
            block_type=cfg.critic_block_type,
            num_blocks=cfg.critic_num_blocks,
            hidden_dim=cfg.critic_hidden_dim,
            dtype=compute_dtype,
            use_post_layernorm=cfg.critic_use_post_layernorm,
            use_post_linear=cfg.critic_use_post_linear,
            post_hidden_dim=cfg.critic_post_hidden_dim,
            use_simnorm=cfg.critic_use_simnorm,
            simnorm_levels=cfg.critic_simnorm_levels,
            use_project_linear=cfg.critic_use_project_linear,
            project_hidden_dim=cfg.critic_project_hidden_dim,
            use_project_activ=cfg.critic_use_project_activ,
        )
    else:
        critic_network_def = SACDevCritic(
            block_type=cfg.critic_block_type,
            num_blocks=cfg.critic_num_blocks,
            hidden_dim=cfg.critic_hidden_dim,
            dtype=compute_dtype,
    ...
    temperature = Trainer.create(
        network_def=SACDevTemperature(cfg.temp_initial_value),
        network_inputs={
            "rngs": temp_key,
        },
        tx=optax.adamw(
...
class SACDevAgent(BaseAgent):
    def __init__(
        ...
        super(SACDevAgent, self).__init__(
            observation_space,
            action_space,
            cfg,
        )
    ...
    def _init_network(self):
        (
            ...
            self._target_critic,
            self._temperature,
        ) = _init_sac_dev_networks(self._observation_dim, self._action_dim, self._cfg)
    ...
    def sample_actions(
        ...
        self._rng, actions = _sample_sac_dev_actions(
