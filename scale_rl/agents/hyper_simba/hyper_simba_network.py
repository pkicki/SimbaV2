...
from scale_rl.agents.hyper_simba.hyper_simba_layer import (
...
class HyperSimbaActor(nn.Module):
    num_blocks: int
    hidden_dim: int
    action_dim: int
    scaler_init: float
...
class HyperSimbaCritic(nn.Module):
    num_blocks: int
    hidden_dim: int
    min_v: float
    max_v: float
...
class HyperSimbaDoubleCritic(nn.Module):
    """
...
class HyperSimbaTemperature(nn.Module):
